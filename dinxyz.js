...
module.exports = async (dinxyz, m, commands, chatUpdate) => {
    try {
        ...
        let metadata = isGroup ? await dinxyz.groupMetadata(from) : {}
        ...
        let isBotAdmin = isGroup ? groupAdmin.includes(dinxyz.user?.jid) : false
        let isAdmin = isGroup ? groupAdmin.includes(sender) : false
        let isOwner = [dinxyz.user?.jid, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)

        if (options.autoRead) (dinxyz.type == "legacy") ? await dinxyz.chatRead(m.key, 1) : await dinxyz.sendReadReceipt(from, sender, [m.id])
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            let messages = await generateWAMessage(m.from, { text, mentions }, {
                userJid: dinxyz.user.jid,
                quoted: m.quoted && m.quoted.fakeObj
            })
            messages.key.fromMe = await areJidsSameUser(m.sender, dinxyz.user.jid)
            ...
            dinxyz.ev.emit("messages.upsert", msg)
        }
        ...
        if (m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
            ...
            let msg = await dinxyz.serializeM(await Store.loadMessage(key.remoteJid, key.id))
            let teks = `
...
⬡ Type : ${msg.type}
            `
            dinxyz.sendText(m.from, teks, msg, { mentions: [msg.sender] })
            dinxyz.relayMessage(m.from, msg.message, { messageId: msg.id })
        }
        ...
        try {
            ...
            dinxyz.sendMessage(from, { text: String(e) }, { quoted: m })
        }   
    } catch (e) {
        console.log(e)
