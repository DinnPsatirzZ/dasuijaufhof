...
function bind(dinxyz) {
    ...
    dinxyz.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    dinxyz.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    dinxyz.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    dinxyz.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   dinxyz.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    dinxyz.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    dinxyz.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    dinxyz.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    dinxyz.ev.on('chats.update', updates => {
    ...
    dinxyz.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    dinxyz.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**dinxyz.ev.on('messages.update', updates => {
    ...
    dinxyz.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    dinxyz.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    dinxyz.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (dinxyz === null || dinxyz === void 0 ? void 0 : dinxyz.messageInfo(remoteJid, id))
                if (msg) {
