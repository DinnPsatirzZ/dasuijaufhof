...
module.exports = {
    ...
    exec: async(dinxyz, m, { text }) => {
        ...
        dinxyz.sendFile(m.from, fetch.result.media[1].url || fetch.result.media[0].url, "", m, { jpegThumbnail: await fetchBuffer(fetch.result.thumbnail), caption: `⭔ Title : ${fetch.result.tile}\n⭔ Source Url : ${fetch.result.url}` })
    },
    isQuery: true
}