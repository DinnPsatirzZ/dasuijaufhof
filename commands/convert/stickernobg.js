...
module.exports = {
    ...
    exec: async(dinxyz, m, { command, prefix, text, quoted, mime }) => {
        ...
        if (/image/.test(mime)) {
            ...
            let download = await dinxyz.downloadAndSaveMediaMessage(quoted)
            let apikey = await imgbb("8170c546056168c248430201c4bf22b6", download)
            hasil = apikey.display_url
            dinxyz.sendFile(m.from, global.api("zenz", "/convert/sticker-nobg", { url: hasil }, "apikey"), "", m, { asSticker: true, author: global.author, packname: global.packname, categories: ['😄','😊'] })
